...
export const CommentModal: React.FC<CommentModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
      <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl w-full max-w-lg max-h-[85vh] flex flex-col overflow-hidden animate-in zoom-in-95 duration-200">
              ...
              {displayedComments
                ...
                .map((comment) => {
                  ...
                  const RenderComment = ({ item, isReply = false }: { item: any, isReply?: boolean, key?: any }) => {
                    ...
                    return (
                      <div className={`flex gap-3 p-2 rounded-lg transition-colors group ${isReply ? 'ml-10 mt-1 bg-slate-50/50 dark:bg-slate-700/10' : 'hover:bg-slate-50 dark:hover:bg-slate-700/30'}`}>
                        <img src={item.user?.profilePicture || '/default-avatar.png'} alt={item.user?.name} className={`${isReply ? 'w-6 h-6' : 'w-8 h-8'} rounded-full object-cover flex-shrink-0 border border-slate-100 dark:border-slate-700`} />
                        <div className="flex-1 min-w-0">
                          <div className="flex items-center justify-between">
                            <div className="flex items-center gap-2 mb-1">
                              <span className={`font-bold text-slate-900 dark:text-white ${isReply ? 'text-xs' : 'text-sm'}`}>{item.user?.name}</span>
                              <span className="text-[10px] text-slate-500 dark:text-slate-400">{formatTimeAgo(item.date || item.createdAt)}</span>
                            </div>
                          ...
                          {itemIsEditing ? (
                            ...
                            <>
                              <p className={`${isReply ? 'text-xs' : 'text-sm'} text-slate-900 dark:text-slate-100 break-words mb-2 leading-relaxed`}>{item.text}</p>
                              <div className="flex items-center gap-4">
                                <button onClick={() => handleLikeComment(item._id)} className="flex items-center gap-1 text-[10px] font-bold text-slate-500 dark:text-slate-400 hover:text-red-500 transition-colors uppercase tracking-wider">
                                  {itemIsLiked ? <svg className="w-3.5 h-3.5 text-red-500 fill-red-500" viewBox="0 0 20 20"><path fillRule="evenodd" d="M3.172 5.172a4 4 0 015.656 0L10 6.343l1.172-1.171a4 4 0 115.656 5.656L10 17.657l-6.828-6.829a4 4 0 010-5.656z" clipRule="evenodd" /></svg> : <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>}
                                  {itemLikeCount > 0 && <span className={itemIsLiked ? 'text-red-500' : ''}>{itemLikeCount}</span>}
                                  <span>Like</span>
                                </button>
                                {!isReply && (
                                  <button onClick={() => { setReplyTo(item); inputRef.current?.focus(); }} className="text-[10px] font-bold text-slate-500 hover:text-blue-500 transition-colors uppercase tracking-wider">Reply</button>
                                )}
                              </div>
                            </>
                          )}
        ...
        <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 rounded-b-2xl">
          {replyTo && (
            <div className="flex items-center justify-between px-4 py-2 mb-2 bg-slate-50 dark:bg-slate-700/50 rounded-lg animate-in slide-in-from-bottom-2 duration-200">
